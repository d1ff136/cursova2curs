...
int main() {
    ...
    int n;
    cin >> n;
    cin.ignore(); // прибираємо \n, який залишився після зчитування
    int** a = new int* [n];
    for (int i = 0; i < n; ++i) {
        a[i] = new int[n];
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            char c; //для зберігання символа
            cin >> c;
            //клітина вільна?
            if (c == '.') {
                a[i][j] = 0;
                //перевіряємо символ без вилучення
                if (cin.peek() >= '0' && cin.peek() <= '9') {
                    cin >> c;//якщо після крапки йде цифра, читаємо
                    a[i][j] = c - '0'; //конвертуємо символ цифри в числове значення
                }
            }
            else {
                a[i][j] = c - '0';
